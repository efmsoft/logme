...
| Sink-level filtering | Channel-level filtering by level and active state, subsystem allow/block rules, optional named-subsystem level overrides, links, and overrides | `logme/include/Logme/Channel.h`, `logme/include/Logme/Logger.h`, `logme/source/Channel.cpp`, `logme/include/Logme/Detail/Precheck.h` | This is intentional. A logme channel is the routing unit, while a named subsystem may replace the channel level for targeted diagnostics. The fast path still does not enumerate backends to decide whether a record should be emitted. |
...
| Per-scope precheck condition / custom should-log predicate | `LoggerCondition()`, overridable per class via a same-named member (mirrors `CH`/`SUBSID` resolution) | `logme/include/Logme/Logger.h`, `logme/include/Logme/Logme.h`, `examples/LoggerConditionOverride`, `tests/LoggerConditionOverride` | Every logging macro calls the unqualified, global `LoggerCondition()` instead of `Logme::Instance->Condition()` directly, so a class can hide the default for its own methods via ordinary C++ member lookup -- no virtual dispatch, no effect on callers that don't opt in. A qualified `Logme::LoggerCondition()` call could not be shadowed this way. Distinct from `Logger::SetCondition()`, which replaces the process-wide default itself. |
| Thread-scoped precheck condition | `LogmeThreadCondition()`, `Logme::ThreadCondition`, `Logger::SetThreadLogCondition`/`GetThreadLogCondition`, `Logger::IsSubsystemBlocked` | `logme/include/Logme/ThreadCondition.h`, `logme/source/ThreadCondition.cpp`, `logme/include/Logme/Logger.h`, `examples/ThreadLogCondition`, `tests/ThreadLogCondition` | Folded into the global `LoggerCondition()` default alongside `Logme::Instance->Condition()`, so it only ever applies to code that does not already declare its own `LoggerCondition()` member override. Lets code with no per-object state (e.g. a shared, parsed rule/DSL tree walked on behalf of many different requests) declare, once per stretch of work like `LogmeThreadSubsystem` declares a subsystem, that every log call made on the current thread until the scope ends can be skipped. `IsSubsystemBlocked` exposes the same blocked/allowed-list check `DoLog()` does per record as a standalone query, so that decision can drive the thread condition once instead of being re-checked (under a lock) on every call. |
...
| On-demand log-source profiling | `logstat` source-site, channel, backend-output and asynchronous file-runtime reports | `logme/source/LogStatistics.cpp`, `logme/source/Control/Command/CmdLogStatistics.cpp`, `docs/log_statistics.md`, `tests/LogStatistics` | Attributes logging load to individual C/C++ call sites, follows routing and fan-out to built-in backends, and reports file-worker batching, write failures and queue drops. Collection is disabled by default and the inactive path avoids registration or counter updates. |
